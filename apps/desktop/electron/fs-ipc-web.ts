...
type IpcMainHandlerRegistry = Pick<typeof ipcMain, 'handle'>
...
export function registerFsIpc({
  ...
  resolveGitBinary,
  ipcMainWeb
}: FsIpcDeps, registry: IpcMainHandlerRegistry = ipcMain) {
  ipcMainWeb.handle('hermes:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMainWeb.handle('hermes:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMainWeb.handle('hermes:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMainWeb.handle('hermes:fs:openDir', async (_event, dirPath) => {
  ...
  ipcMainWeb.handle('hermes:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))
  ...
  ipcMainWeb.handle('hermes:fs:logsRoot', async () => localPluginsRoot('logs'))
  ...
  ipcMainWeb.handle('hermes:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMainWeb.handle('hermes:plugin:probe', async (_event, payload) => {
  ...
  ipcMainWeb.handle('hermes:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMainWeb.handle('hermes:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMainWeb.handle('hermes:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMainWeb.handle('hermes:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
